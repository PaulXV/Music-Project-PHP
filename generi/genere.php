...
<div class="container">
    <form method="get" action="add-save.php">
        ...
        <input type="submit" value="Aggiungi il genere" class="btn btn-success">
    </form><br>
    <form method="get" action="remove-save.php">
        <label for="genere">Nome del genere: </label><br>
        <select name="genere" id="genere">
            <?php
            include '../functions.php';
            $recordsGenere=getRecordsGeneriAssociative();
            foreach ($recordsGenere as $genere){
                echo '<option value="'.$genere["idGenere"].'">'.$genere["nomeGenere"].'</option>';
            }
            ?>
        </select>
        <a href="../index.php" class="btn btn-success">Torna alla homepage</a>
        <input type="submit" value="Rimuovi il genere" class="btn btn-danger">
    </form>
</div>
